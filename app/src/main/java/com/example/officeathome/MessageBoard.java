...
public class MessageBoard extends Activity implements View.OnClickListener {
    ...
    private void initView() {
        ...
        if(comment_list == null){
            Log.d("LOG_TAG", "null pointer");
            return;
        }
        else{
            Log.d("LOG_TAG", "not null pointer");
        }
        // 初始化数据
        data = new ArrayList<>();
        // 初始化适配器
    ...
    public void sendComment(){
        if(comment_content.getText().toString().equals("")){
            Toast.makeText(getApplicationContext(), "Comments cannot be blank！", Toast.LENGTH_SHORT).show();
        }else{
            // 生成评论数据
            Comment comment = new Comment();
            comment.setName("Commenter"+(data.size()+1)+"：");
